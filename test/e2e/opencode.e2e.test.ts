 ...
 *
 * Run with: RUN_E2E_TESTS=true npm run test:run -- test/e2e/opencode.e2e.test.ts
 */
...
describe.skipIf(!RUN_E2E_TESTS)("E2E: OpenCode Agent", () => {
  ...
  beforeEach(() => {
    ...
    tempDir = fs.mkdtempSync(os.tmpdir() + "/opencode-e2e-");
  });
  ...
  describe("agent initialization", () => {
    it("should have opencode registered in factory", () => {
    ...
    it("should spawn opencode agent successfully", () => {
    ...
    it("should advertise capabilities", () => {
      ...
      console.log("OpenCode capabilities:", JSON.stringify(handle.capabilities, null, 2));
  ...
  describe("basic prompting", () => {
    ...
    beforeAll(async () => {
      ...
      promptTempDir = fs.mkdtempSync(os.tmpdir() + "/opencode-prompt-e2e-");
  ...
  describe("session properties", () => {
    it("should have modes property", async () => {
      ...
      // Log for debugging
      console.log("OpenCode session modes:", session.modes);
    ...
    it("should have models property", async () => {
      ...
      // Log for debugging
      console.log("OpenCode session models:", session.models);
  ...
  describe("loadSession capability", () => {
    it("should check loadSession capability", () => {
      // Log the capability
      console.log("OpenCode loadSession capability:", handle.capabilities.loadSession);

      // This documents what OpenCode supports
  ...
  describe("fork capability", () => {
    it("should check fork capability", () => {
      // Log the capability
      console.log(
        "OpenCode fork capability:",
        handle.capabilities.sessionCapabilities?.fork
      );

      // This documents what OpenCode supports
      // Fork capability may or may not be defined
      expect(handle.capabilities).toBeDefined();
    });
  });
});
...
describe.skipIf(!RUN_E2E_TESTS)("E2E: All Agents Comparison", () => {
  it("should have all four agents registered", () => {
  ...
  it("should spawn opencode agent successfully", async () => {
    const opencodeHandle = await AgentFactory.spawn("opencode");
    expect(opencodeHandle).toBeDefined();
    expect(opencodeHandle.capabilities).toBeDefined();

    await opencodeHandle.close();
  }, 120000);
});
